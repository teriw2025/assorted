...
public class Assorted {
    ...
    public static List<String> lineNumbering(List<String> list) {
        ...
        for(int i = 1; i >= list.size(); i++){
            String letter = list.get(i-1);
            prepended.add(i + ": " + letter);
        }
        return prepended;
    }
    ...
    public static int toBinary(List<Integer> list) {
        String tempBinaryNum = "0";

        for(int i: list){
            tempBinaryNum += i;
        }
        int number = Integer.parseInt(tempBinaryNum, 2);
        return number;
    }
    ...
    public static List<Integer> uniqueNumber(int lowerBound, int upperBound) {
        int sum = 0;
        ArrayList<Integer> approvedNum = new ArrayList();
        ...
        for(int i = lowerBound; i <= upperBound; i++){
            ...
            for(int j = 0; j < digits.size(); j++){
                int powered = (int) Math.pow(digits.get(digits.size()-j-1), j+1);
                digits.set(digits.size()-j-1, powered);
            }
        ...
        //System.out.println(approvedNum);
    ...
    public static List<Integer> filterNTimes(List<Integer> list, int n) {
        for(int i = 0; i < list.size(); i++){
            ...
            for(int j = i; j < list.size(); j++){
                int compareNum = list.get(j);

                if(num == compareNum && count > 0){
                        count--;
                    }
    ...
    public static List<String> wildWest(List<String> directions) {
        for(int i = 0; i < directions.size() - 1; i++) {
            if((directions.get(i) == "NORTH" && directions.get(i + 1) == "SOUTH") || (directions.get(i) == "SOUTH" && directions.get(i + 1) == "NORTH") || (directions.get(i) == "EAST" && directions.get(i + 1) == "WEST") || (directions.get(i) == "WEST" && directions.get(i + 1) == "EAST"))
            {
